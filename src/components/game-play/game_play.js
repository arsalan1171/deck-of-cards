...
const Game=({ players,roomId })=> {
  ...
  const handleNextTurn = async ()=> {
    ...
    if (cardValue === 'ACE' || cardValue === 'JACK' || cardValue === 'QUEEN' || cardValue === 'KING') {
      score = 10;
    } else {
  ...
  return (
    <div style={{display:'inline-grid'}}>
      <p>It's {players[currentPlayerIndex]?.name}'s turn. Time left: {timeLeft}</p>
      <Button style={{marginBottom:'20px'}} onClick={handleNextTurn}>draw card</Button>
      {winner && <p>{winner.name} is the winner with a score of {winner.score}!</p>}
      {showAlert && (
        <div>
          {winner ? (
            <p>Game over! {winner.name} wins with a score of {winner.score}!</p>
          ) : (
